...
{
    public partial class ItemTypeProfile : System.Web.UI.Page
    {
        ...
        protected void InsertNewWCItem(OrderedDictionary newValues)
        {
            ...
            var cust = db.ItemTypes.Where(x => x.Code == code).FirstOrDefault();
            if (cust != null)
            {
                return;
            }
            ...
            db.ItemTypes.InsertOnSubmit(item);
            db.SubmitChanges();
            grid.DataBind();
        ...
        protected void DeleteWCItem(OrderedDictionary keys, OrderedDictionary values)
        {
            ...
            var cust = db.ItemTypes.Where(x => x.Code == keys[0].ToString()).FirstOrDefault();
            if (cust == null)
            {
                return;
            }
            db.ItemTypes.DeleteOnSubmit(cust);
            db.SubmitChanges();
            grid.DataBind();
        }
