...
{
    public partial class ItemCategoryProfile : System.Web.UI.Page
    {
        ...
        protected void InsertNewWCItem(OrderedDictionary newValues)
        {
            ...
            var cust = db.ItemCategories.Where(x => x.Code == code).FirstOrDefault();
            if (cust != null)
            {
                return;
            }
            ...
            db.ItemCategories.InsertOnSubmit(item);
            db.SubmitChanges();
            grid.DataBind();
        ...
        protected void DeleteWCItem(OrderedDictionary keys, OrderedDictionary values)
        {
            ...
            var cust = db.ItemCategories.Where(x => x.Code == keys[0].ToString()).FirstOrDefault();
            if (cust == null)
            {
                return;
            }
            db.ItemCategories.DeleteOnSubmit(cust);
            db.SubmitChanges();
            grid.DataBind();
        }
